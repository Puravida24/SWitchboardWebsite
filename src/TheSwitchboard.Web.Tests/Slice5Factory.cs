...
public class Slice5Factory : SwitchboardWebApplicationFactory
{
    private readonly string _dbName = "Slice5-" + Guid.NewGuid();
    ...
    public async Task<HttpClient> LoggedInClientAsync()
    {
        ...
        if (res.StatusCode != HttpStatusCode.Redirect)
            throw new InvalidOperationException("Slice5Factory: admin login did not redirect");
